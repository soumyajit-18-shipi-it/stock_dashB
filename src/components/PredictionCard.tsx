...
export function PredictionCard({ prediction, metrics }: PredictionCardProps) {
  ...
  return (
    <div className="bg-slate-800/50 rounded-xl border border-slate-700 p-6">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        ...
        <div className="bg-slate-700/50 rounded-lg p-4">
          <p className="text-slate-400 text-sm mb-1">Trend Direction</p>
          <div className={`flex items-center gap-2 ${isUp ? 'text-emerald-400' : 'text-red-400'}`}>
            ...
            <span className="text-xl font-bold capitalize">{prediction.trend}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between text-xs text-slate-400 pt-4 border-t border-slate-700">
        <span>Model: <span className="text-slate-300">{prediction.model_used === 'rf' ? 'Random Forest' : 'Linear Regression'}</span></span>
      </div>
