...
function getFallbackContent(title: string, stock: StockResponse, currency: string): string {
  ...
  const currentPrice = stock.profile.current_price
    ? `${currency} ${stock.profile.current_price.toLocaleString()}`
    : 'N/A';
  const prevClose = stock.profile.previous_close
    ? `${currency} ${stock.profile.previous_close.toLocaleString()}`
    : 'N/A';
  const high52 = stock.profile.week_52_high
    ? `${currency} ${stock.profile.week_52_high.toLocaleString()}`
    : 'N/A';
  const low52 = stock.profile.week_52_low
    ? `${currency} ${stock.profile.week_52_low.toLocaleString()}`
    : 'N/A';
  const predictedPrice = stock.prediction.predicted_price
    ? `${currency} ${stock.prediction.predicted_price.toLocaleString()}`
    : 'N/A';
  ...
  const marketCap = stock.profile.market_cap
    ? `${currency} ${(stock.profile.market_cap / 1e9).toFixed(2)}B`
    : 'N/A';
  ...
  const exportReport = async () => {
    ...
    try {
      while (attempts < maxAttempts && !success) {
        ...
        setReport('');
        try {
          console.info(`Attempt ${attempts} of ${maxAttempts}...`);
      ...
      if (!success) {
        showToast(
          'Report generation partially completed. Used data-driven fallbacks for missing sections.'
        );
      }
  ...
  return (
    <>
      <button
        ...
        disabled={loading}
        className="flex items-center gap-2 rounded-lg bg-slate-700 px-4 py-2 text-white transition-colors hover:bg-slate-600 disabled:opacity-60"
      >
      ...
      <div
        className="pointer-events-none fixed -left-[9999px] top-0 w-[850px] bg-white p-12 text-slate-900"
        ref={reportRef}
      >
        <div className="mb-8 flex items-end justify-between border-b-4 border-slate-900 pb-6">
          <div>
            ...
            <p className="text-xl font-bold text-slate-600">
              {stockData.profile.name || stockData.profile.ticker} ({stockData.profile.ticker})
            </p>
          </div>
          <div className="text-right">
            ...
            <p className="text-xs font-black text-red-600 tracking-widest uppercase mt-1">
        ...
        <div className="mb-10 grid grid-cols-3 gap-6 rounded-xl bg-slate-50 p-6 border border-slate-200">
          <div className="space-y-1">
            ...
            <p className="text-sm">
              <strong>Sector:</strong> {stockData.profile.sector || 'N/A'}
            </p>
            <p className="text-sm">
              <strong>Industry:</strong> {stockData.profile.industry || 'N/A'}
            </p>
            <p className="text-sm">
              <strong>Exchange:</strong> {stockData.profile.exchange || 'N/A'}
            </p>
          </div>
          <div className="space-y-1 border-x border-slate-200 px-6">
            ...
            <p className="text-sm">
              <strong>Price:</strong> {currency} {stockData.profile.current_price?.toLocaleString()}
            </p>
            <p className="text-sm">
              <strong>Market Cap:</strong>{' '}
              {stockData.profile.market_cap
                ? `${currency} ${(stockData.profile.market_cap / 1e9).toFixed(2)}B`
                : 'N/A'}
            </p>
            <p className="text-sm">
          ...
          <div className="space-y-1 pl-6">
            ...
            <p className="text-sm">
              <strong>Predicted:</strong> {currency}{' '}
              {stockData.prediction.predicted_price?.toLocaleString()}
            </p>
            <p className="text-sm">
              <strong>Confidence:</strong> {(stockData.prediction.confidence * 100).toFixed(1)}%
            </p>
            <p className="text-sm">
              <strong>Trend:</strong>{' '}
              <span
                className={
                  stockData.prediction.trend === 'increase' ? 'text-emerald-600' : 'text-rose-600'
                }
              >
                {stockData.prediction.trend?.toUpperCase()}
              </span>
            </p>
          </div>
        </div>
          ...
          {sections.map((section) => (
            <section key={section.title} className="break-inside-avoid">
              ...
              <div className="whitespace-pre-wrap text-[13px] leading-relaxed text-slate-700 font-medium">
                {section.content ||
                  (generationComplete
                    ? 'Section generation incomplete. Model token limit reached or generation failed.'
        ...
        <div className="mt-12 border-t border-slate-200 pt-8">
          ...
          <p className="mt-4 text-[9px] text-slate-400 text-justify leading-tight italic">
            ...
            financial advice. The analysis is generated using machine learning models and artificial
            intelligence based on historical data. Investing in securities involves risk. Always
