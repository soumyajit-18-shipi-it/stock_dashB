  ...
  const handleTest = async () => {
    ...
    try {
      ...
      if (mountedRef.current)
        setStatus(message === 'Timeout' ? t('timeout') : `${t('connectionFailed')}: ${message}`);
    } finally {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/80 p-4">
      <div className="w-full max-w-lg rounded-xl border border-slate-700 bg-slate-900 p-6 shadow-2xl">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4">
            ...
            {models.length > 0 ? (
              ...
              <p className="mt-1 text-sm font-medium text-white italic">
                {detecting ? t('detectingModels') : form.selectedModel || t('notAvailable')}
              </p>
            )}
            <p className="mt-2 text-xs text-slate-400">
              Models are detected automatically from your provider.
            </p>
          </div>

          {status && (
            <div
              className={`p-3 rounded-lg text-sm ${status.includes('success') || status.includes(t('connected')) ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}
            >
              {status}
            </div>
          )}
        ...
        <div className="mt-8 flex justify-end gap-3">
          <button
            onClick={handleTest}
            disabled={testing || detecting || !canFetchModels(form)}
            className="rounded-lg bg-slate-700 px-4 py-2 text-white hover:bg-slate-600 disabled:opacity-50 transition-colors"
          >
          ...
          <button
            onClick={handleSave}
            disabled={detecting}
            className="rounded-lg bg-emerald-600 px-6 py-2 font-semibold text-white hover:bg-emerald-700 disabled:opacity-50 transition-colors shadow-lg shadow-emerald-900/20"
          >
            {t('save')}
          </button>
        </div>
