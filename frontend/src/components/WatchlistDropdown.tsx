...
export function WatchlistDropdown() {
  ...
  const filteredWatchlist = useMemo(() => {
    ...
    return watchlist.filter((item) => item.ticker.includes(term) || (item.name || '').toUpperCase().includes(term));
  }, [search, watchlist]);
      ...
      {open && (
        <div className="fixed inset-0 z-[80] overflow-y-auto border-slate-700 bg-slate-900 p-4 shadow-2xl sm:absolute sm:inset-auto sm:right-0 sm:top-full sm:mt-2 sm:w-[28rem] sm:rounded-xl sm:border">
          <div className="mb-4 flex items-center justify-between">
            ...
            <button className="rounded-lg px-2 py-1 text-sm text-slate-400 hover:bg-slate-800 hover:text-white sm:hidden" onClick={() => setOpen(false)}>
              {t('cancel')}
            </button>
          </div>
          ...
          <div className="mb-4 flex gap-2">
            ...
            <button onClick={handleAdd} className="inline-flex items-center gap-1 rounded-lg bg-emerald-600 px-3 py-2 text-sm font-medium text-white hover:bg-emerald-700">
          ...
          <div className="space-y-2">
            {isLoading ? (
              ...
              <p className="rounded-lg bg-slate-800/50 p-4 text-center text-sm text-slate-400">{t('noStocks')}</p>
            ) : filteredWatchlist.length === 0 ? (
              <p className="rounded-lg bg-slate-800/50 p-4 text-center text-sm text-slate-400">{t('notAvailable')}</p>
              ...
              const change = price != null && previous ? ((price - previous) / previous) * 100 : null;
              const values = quote?.history?.slice(-12).map((point) => point.close) || [];
              return (
                <div key={item.id || item.ticker} className={`rounded-lg border p-3 ${ticker === item.ticker ? 'border-emerald-500/30 bg-emerald-600/10' : 'border-slate-700 bg-slate-800/50'}`}>
                  <div className="flex items-center justify-between gap-3">
                    <button onClick={() => handleSelect(item.ticker)} className="min-w-0 flex-1 text-left">
                      <p className="font-medium text-white">{item.ticker}</p>
                      <p className="truncate text-xs text-slate-400">{profile?.name || item.name || t('notAvailable')}</p>
                    </button>
                    <div className="text-right">
                      <p className="text-sm font-semibold text-white">{price != null ? price.toFixed(2) : '--'}</p>
                      <p className={`text-xs ${change == null || change >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{change == null ? '--' : `${change.toFixed(2)}%`}</p>
                    </div>
                    <Sparkline values={values} />
                    {item.id && (
                      <button onClick={() => void remove(item.id!)} className="rounded p-1 hover:bg-red-500/20" aria-label={t('removeTicker')}>
          ...
          <div className="mt-4 border-t border-slate-700 pt-3">
            ...
            <div className="flex flex-wrap gap-2">
              {history.slice(0, 6).map((item) => (
                <button key={`${item.ticker}-${item.searched_at || item.id}`} onClick={() => handleSelect(item.ticker)} className="rounded-full bg-slate-800 px-3 py-1 text-xs text-slate-300 hover:bg-slate-700">
                  {item.ticker}
                </button>
              ))}
              {history.length === 0 && <p className="text-xs text-slate-400">{t('notAvailable')}</p>}
            </div>
          </div>
        </div>
      )}
