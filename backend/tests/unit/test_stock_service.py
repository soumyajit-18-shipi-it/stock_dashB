...
async def test_stock_service_full_analysis() -> None:
    ...
    svc.data_provider.get_stock_data = MagicMock(return_value=mock_df)
    svc.data_provider.last_latency = 100.0
    svc.data_provider.get_company_info = MagicMock(return_value={"marketCap": 1000000})

    # Mock finnhub_service
    mock_finnhub = {"name": "Test Inc.", "marketCapitalization": 1.0}
    with patch("services.stock_service.finnhub_service") as mock_finnhub_svc:
        ...
        svc.predictor.predict = MagicMock(return_value=(mock_pred, mock_metrics))
