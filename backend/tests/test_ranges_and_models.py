...
def test_chart_ranges_return_different_counts(provider: StockDataProvider) -> None:
    # Mock yfinance chart fetch
    mock_ticker = MagicMock()

    # We define history generator for different ranges
    def mock_history(period: str, interval: str = "1d") -> pd.DataFrame:
    ...
    mock_fast_info.__getitem__.side_effect = lambda key: getattr(
        mock_fast_info, key, None
    )
...
def test_models_train_and_predict_independently(predictor: StockPredictor) -> None:
